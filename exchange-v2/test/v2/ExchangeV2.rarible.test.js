...
contract("ExchangeV2, sellerFee + buyerFee =  6%,", (accounts) => {
  ...
  describe("matchOrders", () => {
      ...
      await verifyBalanceChange(accounts[7], 206, async () =>
        verifyBalanceChange(accounts[1], -194, async () =>
          verifyBalanceChange(protocol, -12, () =>
            ...
            testing.matchOrders(left, signatureLeft, right, signatureRight, {
              from: accounts[7],
              value: 300,
              gasPrice: 0,
            })
    ...
    it("ERC721 to ETH order maker ETH != who pay, ETH orders have no signature, throw", async () => {
      ...
      await expectThrow(
        testing.matchOrders(left, signatureLeft, right, "0x", {
          from: accounts[7],
          value: 300,
          gasPrice: 0,
        })
    ...
    it("should match orders with ERC721 сollections", async () => {
      ...
      const right = Order(
        ...
        Asset(ETH, "0x", 200),
        ZERO,
        Asset(COLLECTION, enc(erc721.address), 1),
        1,
        0,
        0,
        "0xffffffff",
  ...
  describe("Do matchOrders(), orders dataType == V1", () => {
    ...
    async function prepare2Orders(
      ...
      await t2.approve(erc20TransferProxy.address, 10000000, {
        from: accounts[2],
      });
      let addrOriginLeft = [[accounts[3], makeAmount]];
      let addrOriginRight = [[accounts[4], takeAmount]];
      let encDataLeft = await encDataV1([
        [[accounts[1], 10000]],
        addrOriginLeft,
      ]);
      let encDataRight = await encDataV1([
        [[accounts[2], 10000]],
        addrOriginRight,
      ]);
      const left = Order(
        accounts[1],
        Asset(ERC20, enc(t1.address), makeAmount),
        ZERO,
        Asset(ERC20, enc(t2.address), takeAmount),
        1,
        0,
        0,
        ORDER_DATA_V1,
    ...
    async function prepare20DV1_1155V2Orders(t1Amount = 120, t2Amount = 10) {
      ...
      await erc1155_v2.setApprovalForAll(transferProxy.address, true, {
        from: accounts[2],
      });
      ...
      await royaltiesRegistry.setRoyaltiesByToken(erc1155_v2.address, [
        [accounts[6], 1000],
        [accounts[7], 500],
      ...
      const left = Order(
        ...
        Asset(ERC20, enc(t1.address), 100),
        ZERO,
        Asset(ERC1155, enc(erc1155_v2.address, erc1155TokenId1), 7),
        1,
        0,
        0,
        ORDER_DATA_V1,
    ...
    it("From ERC1155(RoyalytiV2, DataV1) to ERC20(DataV1):Protocol, Origin fees, Royalties ", async () => {
      ...
      await testing.matchOrders(
        left,
        await getSignature(left, accounts[2]),
        right,
        "0x",
        { from: accounts[1] }
      );
    ...
    it("From ETH(DataV1) to ERC720(RoyalytiV1, DataV1) Protocol, Origin fees, Royalties", async () => {
      ...
      await royaltiesRegistry.setRoyaltiesByToken(erc721V1.address, [
        [accounts[3], 300],
        [accounts[4], 400],
      ]); //set royalties by token
      const left = Order(
        ...
        Asset(ETH, "0x", 200),
        ZERO,
        Asset(ERC721, enc(erc721V1.address, erc721TokenId1), 1),
        1,
        0,
        0,
        ORDER_DATA_V1,
      ...
      let signatureRight = await getSignature(right, accounts[1]);
      await verifyBalanceChange(accounts[2], 228, async () =>
        //200+6buyerFee+ (10+12 origin left) (72back)
        verifyBalanceChange(accounts[1], -166, async () =>
          //200 -6seller - (6+8royalties) - 14originright
    ...
    it("From ETH(DataV1) to ERC720(DataV1) Protocol, Origin fees,  no Royalties", async () => {
      ...
      await verifyBalanceChange(accounts[2], 228, async () =>
        //200+6buyerFee+ (10 +12 origin left) (72back)
        verifyBalanceChange(accounts[1], -180, async () =>
    ...
    it("From ETH(DataV1) to ERC720(DataV1) Protocol, Origin fees comes from OrderNFT,  no Royalties", async () => {
      ...
      await verifyBalanceChange(accounts[2], 206, async () =>
        ...
        verifyBalanceChange(accounts[1], -158, async () =>
    ...
    it("From ETH(DataV1) to ERC720(DataV1) Protocol, Origin fees comes from OrderETH,  no Royalties", async () => {
      ...
      let signatureRight = await getSignature(right, accounts[1]);
      await verifyBalanceChange(accounts[2], 242, async () =>
    ...
    it("From ETH(DataV1) to ERC720(DataV1) Protocol, no Royalties, Origin fees comes from OrderETH NB!!! not enough ETH", async () => {
      ...
      await expectThrow(
        testing.matchOrders(
          ...
          right,
          await getSignature(right, accounts[1]),
          { from: accounts[2], value: 300, gasPrice: 0 }
        )
      );
  ...
  describe("Do matchOrders(), orders dataType == V1, MultipleBeneficiary", () => {
    ...
    it("From ETH(DataV1) to ERC721(DataV1) Protocol, Origin fees,  no Royalties, payouts: 50/50%", async () => {
      ...
      await verifyBalanceChange(accounts[2], 228, async () =>
        //200+6buyerFee+ (10 +12 origin left) (72back)
        verifyBalanceChange(accounts[3], -90, async () =>
          //200 -6seller - 14 originright *50%
          verifyBalanceChange(accounts[1], -90, async () =>
            //200 -6seller - 14 originright *50%
            verifyBalanceChange(accounts[5], -10, async () =>
              verifyBalanceChange(accounts[6], -12, async () =>
                verifyBalanceChange(accounts[7], -14, async () =>
  ...
  describe("Catch emit event Transfer", () => {
    it("From ETH(DataV1) to ERC721(DataV1) Protocol, check emit ", async () => {
      ...
      let tx = await testing.matchOrders(left, "0x", right, signatureRight, {
        from: buyer,
        value: 300,
        gasPrice: 0,
      });
      ...
      truffleAssert.eventEmitted(
        ...
        (ev) => {
          ...
          switch (ev.to) {
            case protocol:
              if (
                ev.transferDirection != TO_TAKER &&
                ev.transferType != PROTOCOL
              ) {
    ...
    it("From ERC1155(DataV2) to ETH(DataV1) Protocol, check emit ", async () => {
      ...
      const left = Order(
        seller,
        Asset(ERC1155, enc(erc1155_v2.address, erc1155TokenId1), 5),
        ZERO,
        Asset(ETH, "0x", 200),
        1,
        0,
  ...
  describe("Exchange with Royalties", () => {
    it("Royalties by owner, token 721 to ETH", async () => {
      ...
      await royaltiesRegistry.setRoyaltiesByToken(erc721.address, [
        [accounts[3], 500],
        [accounts[4], 1000],
      ]); //set royalties by token
      let addrOriginLeft = [
      ...
      await verifyBalanceChange(accounts[2], 228, async () =>
        //200+6buyerFee+ (10 +12 origin left) (72back)
        verifyBalanceChange(accounts[1], -150, async () =>
          //200 -6seller - 14 originright
    ...
    it("Royalties by token and tokenId 721v1_OwnableUpgradaeble to ETH", async () => {
      ...
      await ERC721_V1OwnUpgrd.setApprovalForAll(transferProxy.address, true, {
        from: accounts[1],
      });
      await royaltiesRegistry.setRoyaltiesByTokenAndTokenId(
      ...
      const left = Order(
        ...
        Asset(ETH, "0x", 200),
        ZERO,
        Asset(ERC721, enc(ERC721_V1OwnUpgrd.address, erc721TokenId1), 1),
        1,
        0,
        0,
        ORDER_DATA_V1,
      ...
      let signatureRight = await getSignature(right, accounts[1]);
      await verifyBalanceChange(accounts[2], 216, async () =>
        //200+6buyerFee+ (10  origin left) (72back)
        verifyBalanceChange(accounts[1], -150, async () =>
          //200 -6seller - 14 originright
          verifyBalanceChange(accounts[3], -10, async () =>
            verifyBalanceChange(accounts[4], -20, async () =>
  ...
  describe("matchOrders, orderType = V2", () => {
    it("should correctly calculate make-side fill for isMakeFill = true ", async () => {
      ...
      const left = Order(
        seller,
        Asset(ERC1155, enc(erc1155_v2.address, erc1155TokenId1), 200),
        ZERO,
        Asset(ETH, "0x", 1000),
        1,
        0,
      ...
      const right = Order(
        ...
        Asset(ETH, "0x", 500),
        ZERO,
        Asset(ERC1155, enc(erc1155_v2.address, erc1155TokenId1), 100),
        1,
        0,
        0,
        ORDER_DATA_V2,
      ...
      await verifyBalanceChange(seller, -485, async () =>
        verifyBalanceChange(buyer, 515, async () =>
          testing.matchOrders(
            ...
            right,
            "0x",
            { from: buyer, value: 600, gasPrice: 0 }
          )
        )
      );
      ...
      const test_hash = await libOrder.hashV2(
        seller,
        Asset(ERC1155, enc(erc1155_v2.address, erc1155TokenId1), 200),
        Asset(ETH, "0x", 1000),
        1,
      ...
      assert.equal(
        await testing.fills(leftOrderHash),
        100,
        "left fill make side"
      );
      ...
      await verifyBalanceChange(seller, -291, async () =>
        verifyBalanceChange(buyer1, 309, async () =>
          testing.matchOrders(
            left1,
            await getSignature(left1, seller),
            right1,
            "0x",
            { from: buyer1, value: 600, gasPrice: 0 }
          )
        )
      );
      assert.equal(
      ...
      await verifyBalanceChange(seller, -970, async () =>
        verifyBalanceChange(buyer1, 1030, async () =>
          testing.matchOrders(
            ...
            right1,
            "0x",
            { from: buyer1, value: 1100, gasPrice: 0 }
          )
        )
      );
